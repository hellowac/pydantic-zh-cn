...
def to_string(value: Any) -> str:
    ...
    if isinstance(value, (dict, list, tuple, set)):
        ...
        re.fullmatch(r, value, flags=re.DOTALL) for r in ['{".+}', r"\[.+\]"]
    ):
        try:
...
def populate_upgraded_versions(
    file: Path, file_text: str, lowest_version: Version
) -> list[tuple[Path, str, Version]]:
    versions = []
    ...
    while minor < HIGHEST_VERSION[1]:
        ...
        if upgraded_file_text != new_file_text:
            ...
            new_file = UPGRADED_TMP_EXAMPLES_DIR / (
                file.stem + f"_{major}_{minor}" + file.suffix
            )
...
def exec_examples() -> int:  # noqa: C901 (I really don't want to decompose it any further)
    ...
    for file in sorted(EXAMPLES_DIR.iterdir()):
        ...
        def error(*desc: str) -> None:
            ...
            filename = Path(previous_frame.f_globals["__file__"]).relative_to(
                Path.cwd()
            )
            location = f"{filename}:{previous_frame.f_lineno}"
            sys.stderr.write(
                f'{location}: error in {file.relative_to(Path.cwd())}:\n{" ".join(desc)}\n'
            )
        ...
        if should_run_as_is:
            ...
            final_content.append("_(这个脚本是完整的，它应该“按原样”运行)_")
        elif requirements:
            # final_content.append(f"_(This script requires {requirements})_")
            final_content.append(f"_(这个脚本需要 {requirements})_")
        else:
            error(
